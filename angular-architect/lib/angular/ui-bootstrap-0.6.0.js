...
.directive('datepickerPopupWrap', [function() {
  return {
    ...
    template: '<ul class="dropdown-menu" ng-style="{display: (isOpen && \'block\') || \'none\', top: position.top+\'px\', left: position.left+\'px\'}">'+
'        <li ng-transclude></li>'+
'        <li class="divider"></li>'+
 ...
 */
  ...
  //                         ing.desc     as   desc     for       ingrediente         in    ingredienti
  ...
  return {
    ...
    link:function (originalScope, element, attrs, modelCtrl) {
	  ...
	  var newscope = originalScope.$eval(attrs.samescope) !== true;
      ...
      if (angular.isDefined(attrs.typeaheadPopuptemplateUrl)) {
        popUpEl.attr('popuptemplate-url', attrs.typeaheadPopuptemplateUrl);
      }
  ...
  .directive('typeaheadPopup', ['$http', '$templateCache', '$compile', '$parse', function ($http, $templateCache, $compile, $parse) {
    return {
      restrict:'E',
      scope:{
        matches:'=',
      ...
      //			'<typeahead-match index="$index" match="match" query="query" template-url="templateUrl"></typeahead-match></li></ul>',
      link:function (scope, element, attrs) { 
    	var tplUrl = attrs.popuptemplateUrl || 'template/typeahead/typeahead-popup.html';       
    	$http.get(tplUrl, {cache: $templateCache}).success(function(tplContent){
             element.replaceWith($compile(tplContent.trim())(scope));
        }).error(function(){
        	var tplContent  = '<ul class="typeahead dropdown-menu" ng-style="{display: isOpen()&&\'block\' || \'none\', top: position.top+\'px\', left: position.left+\'px\'}">'+ 
            			'<li ng-repeat="match in matches" ng-class="{active: isActive($index) }" ng-mouseenter="selectActive($index)" ng-click="selectMatch($index)">'+
            			'<typeahead-match index="$index" match="match" query="query" template-url="templateUrl"></typeahead-match></li></ul>';
        	element.replaceWith($compile(tplContent.trim())(scope));
        });
  ...
  .directive('typeaheadMatch', ['$http', '$templateCache', '$compile', '$parse', function ($http, $templateCache, $compile, $parse) {
    return {
      restrict:'E',
      scope:{
        index:'=', //l'indice della i-esima ripetizione di ng-repeat
        match:'=', //l'iesima ripetizione di ng-repeat
        query:'='  //la stringa di 
      },
      link:function (scope, element, attrs) {
